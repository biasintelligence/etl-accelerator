...
union all select @BatchID,'Workload_workingDir'	,'' --placeholder for workload working dir
...
union all
select 2,@BatchID,'ST011','processor: get workload',32,35,34,null,'011'
union all
...
if (''0'' = @fid)
begin
	--skip all workload steps and exit loop
...
begin
	--set loop properties for the next file to process
	...
	exec dbo.prc_ETLAttributeSet <@BatchID>,null,null,''Workload_fileId'',@fid;
	exec dbo.prc_ETLAttributeSet <@BatchID>,null,null,''Workload_fileName'',@fn;
	exec dbo.prc_ETLAttributeSet <@BatchID>,null,null,''workload_workingDir'',@dir;
end
'
--on failure
...
--on failure
union all select 3,@BatchId,'FileId','<Workload_fileId>' -- workload fileId
